...
Later I tried the bigger route on the same box: GLM-4.5-Air, a 106B mixture-of-experts model (12B active), at 2-bit through llama.cpp, with the attention layers on the 12 GB card and the expert weights in 31 GB of RAM. The 43 GB file does not fit, so part of it is read from the SSD as it runs. Reading prompts ran at about 66 tokens a second once the batch size was raised to 4096 (15 at the default), and writing at about 4. On a small café demo site it did the job in about six minutes: read the right files, edited, built, replied in one plain sentence. But asked for a new headline it changed the café's name in the copy file, which also changes the header, footer and page titles. Opus, on the same site and the same request, changed only the headline in 35 seconds. Without `--bare`, with Claude Code's full 22,000-token prompt, it was worse: six minutes just to read the prompt, thirteen in all, the same renamed café, no build run, and its running commentary ("Let me look for the home page") shown to the person in the chat. On a box this size, bare is the better mode for a local model. And Opus, given the original wording "to X, nothing else", put "nothing else" into the headline too, so that failure in the table above says more about the wording than about the 35B.
